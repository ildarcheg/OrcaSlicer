...
- [x] `stamp_source_if_missing` helper in `project_ops.cpp` provides
  ...
  plus an end-to-end lock-in test on the saved 3mf.
...
- [x] Layer A / Layer B fixture model: `two_cubes.stl` committed
  in-tree (generated by `tests/cli/fixtures/gen_minimal_stls.cpp`),
  `box_with_text.stl` optional local-dev fixture with SKIP-when-absent
  behaviour.
