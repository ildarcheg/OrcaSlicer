...
& build\src\cli\Release\orca-cli.exe project info set $WORK `
    --title "Smoke Phase 10" `
    --description "Generated by Phase 10 recipe" `
    --cover $PNG

# 2. Profile metadata
& build\src\cli\Release\orca-cli.exe project profile set $WORK `
    --title "Profile Smoke" `
    --description "Profile-side description"

# 3. Aux attachments: assembly guide + picture
& build\src\cli\Release\orca-cli.exe project aux add $WORK `
    --folder assembly-guide --file $TXT --name "instructions.txt"
& build\src\cli\Release\orca-cli.exe project aux add $WORK `
    --folder pictures --file $PNG --name "hero.png"

# 4. List to verify
& build\src\cli\Release\orca-cli.exe --json project aux list $WORK
```
